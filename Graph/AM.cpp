...
// ---------- FIND & PRINT PATH ----------
bool findPathUtil(int u, int v, bool visited[], int path[], int &pathIndex) {
    visited[u] = true;
    ...
    for (int i = 0; i < n; i++) {
        if (adjMatrix[u][i] != 0 && !visited[i]) {
            if (findPathUtil(i, v, visited, path, pathIndex))
                return true;
        }
    }

    // BACKTRACK (remove node if not correct path)
    pathIndex--;
    return false;
}
...
void findPath(int u, int v) {
    ...
    if (findPathUtil(u, v, visited, path, pathIndex)) {
        cout << "Path exists: ";
...
// ---------- CYCLE DETECTION ----------
bool isCycleUtilUndirected(int v, bool visited[], int parent) {
    visited[v] = true;

    for (int i = 0; i < n; i++) {
        if (adjMatrix[v][i] != 0) {
            if (!visited[i]) {
                if (isCycleUtilUndirected(i, visited, v))
                    return true;
            }
            else if (i != parent) {
                return true;
...
bool isCycleUtilDirected(int v, bool visited[], bool recStack[]) {
    visited[v] = true;
    recStack[v] = true;

    for (int i = 0; i < n; i++) {
        if (adjMatrix[v][i] != 0) {
            if (!visited[i] && isCycleUtilDirected(i, visited, recStack))
                return true;
            else if (recStack[i])
                return true;
        }
...
bool isCycle() {
    ...
    if (!directed) {
        for (int i = 0; i < n; i++) {
            if (!visited[i]) {
                if (isCycleUtilUndirected(i, visited, -1))
                    return true;
            }
        }
    }
        ...
        for (int i = 0; i < n; i++) {
            if (!visited[i]) {
                if (isCycleUtilDirected(i, visited, recStack))
                    return true;
            }
        }
    }
...
int main() {
    ...
    do {
        ...
        switch(choice) {
            ...
            case 10:
                cout << (isGraphConnected() ? "Connected\n" : "Not Connected\n");
                break;

            case 11:
                cout << (isCycle() ? "Cycle Exists\n" : "No Cycle\n");
                break;
